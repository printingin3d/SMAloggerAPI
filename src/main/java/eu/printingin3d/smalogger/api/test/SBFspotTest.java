 ...
 *
 */
public final class SBFspotTest {
        ...
        try (SmaLogger smaLogger = new SmaLogger()) {
            ...
            try (Inverter inverter = smaLogger.createInverter("192.168.0.12")) {
                ...
                if (labels.getDevClass() == InvDeviceClass.BatteryInverter) {
                    ...
                    System.out.printf("Batt. Temperature: %3.1f%sC\n", batteryInfo.getBatteryTemperature(),
                            Misc.SYM_DEGREE); // degree symbol is different on windows/linux
                ...
                System.out.printf("Device Temperature: %3.1f%sC\n", temperature, Misc.SYM_DEGREE); // degree symbol is
                                                                                                   // different on
                                                                                                   // windows/linux
                ...
                Map<LocalDateTime, DoubleSummaryStatistics> dayData = inverter.getInverterData(new DayDataRequest())
                        .stream().collect(Collectors.groupingBy(d -> d.getDt().truncatedTo(ChronoUnit.HOURS),
